...
def draw_bounding_box_on_image_array(image,
  ...
  """
  image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
  draw_bounding_box_on_image(image_pil, ymin, xmin, ymax, xmax,count, color,
...
def draw_bounding_box_on_image(image,
  ...
  roi=np.array(image.getdata()).reshape((im_height,im_width,3)).astype(np.uint8)
  just=np.random.randn()
  roi=roi[int(top):int(bottom),int(left):int(right)]
  #cv2.imwrite(str(just)+".jpg",roi)
  cv2.imwrite(os.path.join(path, "undereye"+str(count)+".jpg"),roi)
  try:
